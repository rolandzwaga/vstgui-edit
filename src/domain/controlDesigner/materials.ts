/**
 * Control Designer Material Factory
 *
 * Creates Three.js materials for control components.
 * Supports solid, metallic, matte, and brushed metal materials.
 * Migrated and extended from knobDesigner/materials.ts for shared use.
 */

import type { Material } from 'three';
import { Color, DoubleSide, MeshBasicMaterial, MeshStandardMaterial } from 'three';
import type { LayerMaterial } from '../../types/controlDesigner';
...
/**
 * Indicator material configuration (simplified material settings).
 */
export interface SimpleIndicatorMaterial {
...
function getMaterialCacheKey(
  material: LayerMaterial | SimpleIndicatorMaterial,
  isIndicator: boolean
): string {
  if (isIndicator) {
    const ind = material as SimpleIndicatorMaterial;
 ...
 *
 * @param hex - Color in #RRGGBBAA or #RRGGBB format
 * @returns Object with color and alpha
 */
export function parseColor(hex: string): { color: Color; alpha: number } {
  // Remove # if present
 ...
 * @returns Three.js Material
 */
export function createIndicatorMaterial(material: SimpleIndicatorMaterial): Material {
...
// ============================================================================
// Glow Material (for slider value fill)
// ============================================================================

/**
 * Creates a glow material for slider value fill.
 *
 * @param color - Fill color in hex format
 * @param glowIntensity - Glow intensity (0-100)
 * @returns MeshStandardMaterial with emissive glow
 */
export function createGlowMaterial(color: string, glowIntensity: number): MeshStandardMaterial {
  const { color: threeColor, alpha } = parseColor(color);

  const emissiveIntensity = (glowIntensity / 100) * 2;
...
/**
 * Creates a default layer material configuration.
 *
 * @param color - Base color (defaults to gray)
 * @returns Default LayerMaterial configuration
 */
export function createDefaultMaterial(color = '#808080FF'): LayerMaterial {
  return {
    type: 'metallic',
    color,
    shininess: 60,
...
/**
 * Preset material configurations for common appearances.
 */
export const MATERIAL_PRESETS = {
  /** Brushed aluminum - professional audio equipment */
  brushedAluminum: {
    type: 'brushed' as const,
    color: '#A8A8A8FF',
    shininess: 90,
    reflectivity: 70,
    brushDirection: 'radial' as const,
    brushIntensity: 50,
  },
  ...
  /** Wood - warm natural tone */
  wood: {
    type: 'solid' as const,
    color: '#8B4513FF',
    shininess: 20,
    reflectivity: 10,
    brushDirection: 'linear' as const,
    brushIntensity: 30,
  },
} satisfies Record<string, LayerMaterial>;
